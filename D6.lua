...
getgenv().AF_SpawnMainUI = function()
    ...
    getgenv().AF_MainData = { TB=TB, MM=MM, MS=MS, T=T, LS=LS, FTB=FTB, BTS=BTS, FAST=FAST, FASTS=FASTS, AFK=AFK, AFKS=AFKS, AB=AB, AS=AS, SC=SC, ST=ST, ISG=ISG, IF=IF, SR=SR, SGld=SGld, MainSG=MainSG }
                end
...
if getgenv().AF_SubBtn then
    getgenv().AF_SubBtn.MouseButton1Click:Connect(function()
        if getgenv().AF_TI and getgenv().AF_TI.Text == "Антифрикбог" then 
            ...
            local G1, G1S = makeGear("Gear 1", UDim2.new(0, 0, 0, 20), false, UI.SC)
            local G2, G2S = makeGear("Gear 2", UDim2.new(0, 75, 0, 20), true, UI.SC)
            local G3, G3S = makeGear("Gear 3", UDim2.new(0, 150, 0, 20), false, UI.SC)
            ...
            task.spawn(function() 
                ...
                if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then 
                    ...
                    UI.SR.Text = "Runs Completed: 0"
                    UI.SGld.Text = "Gold Earned: 0"
                    UI.ISG.Enabled = true
            ...
            RS.Stepped:Connect(function()
                ...
                if hrp and getgenv().TreasureAutoFarm.Mode == "Fly-TP" then 
                    ...
                    hrp.Anchored = false
                                ...
                                ta.sk.spawn(fun.ction()
            wh.ile tr.ue do
                ta.sk.wait(0.3)
                lo.cal char = LP.Character
                lo.cal hrp = char and char:FindFirstChild("HumanoidRootPart")
                lo.cal hum = char and char:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health > 0 th.en
                    if getgenv().TreasureAutoFarm.Mode == "Fly-TP" th.en
                        pcall(fun.ction()
                            lo.cal ns = WS:WaitForChild("BoatStages", 10):WaitForChild("NormalStages", 10)
                            lo.cal startPos = Vector3.new(-50, 85, -1025)
                            lo.cal st1 = ns:FindFirstChild("CaveStage1")
                            if st1 and st1:FindFirstChild("DarknessPart") th.en 
                                startPos = Vector3.new(st1.DarknessPart.Position.X, 85, st1.DarknessPart.Position.Z) 
                                pcall(fun.ction() firetouchinterest(hrp, st1.DarknessPart, 0); firetouchinterest(hrp, st1.DarknessPart, 1) en.d)
                            en.d
                            hrp.CFrame = CFrame.new(startPos); ta.sk.wait(0.3)
                            for i = 2, 10 do
                                if getgenv().TreasureAutoFarm.Mode ~= "Fly-TP" th.en break en.d
                                lo.cal stage = ns:FindFirstChild("CaveStage" .. i)
                                lo.cal stagePos = (stage and stage:FindFirstChild("DarknessPart")) and stage.DarknessPart.Position or Vector3.new(-50, 85, -1025 + (i * -775))
                                hybridMoveTo(stagePos, i)
                                if (getgenv().TreasureAutoFarm.SpeedTier or 2) < 3 and (i == 3 or i == 6 or i == 9 or i == 10) th.en
                                ...
                                en.d
                            en.d
                            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" and ns:FindFirstChild("TheEnd") th.en
                                lo.cal chest = ns.TheEnd:FindFirstChild("GoldenChest")
                                lo.cal trig = chest and chest:FindFirstChild("Trigger")
                                if trig th.en hrp.CFrame = CFrame.new(trig.Position); pcall(fun.ction() for i = 1, 15 do firetouchinterest(hrp, trig, 0); firetouchinterest(hrp, trig, 1) en.d en.d)
                                el.se hrp.CFrame = CFrame.new(-50, -15, -9200) en.d
                                ta.sk.wait(getgenv().TreasureAutoFarm.WaitAtChest)
                            en.d
                            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" th.en
                                lo.cal respawned = false
                                lo.cal conn = LP.CharacterAdded:Connect(fun.ction() respawned = tr.ue en.d)
                                getgenv().AF_TotalRuns = getgenv().AF_TotalRuns + 1; getgenv().AF_TotalGold = getgenv().AF_TotalGold + 105
                                pcall(fun.ction() UI.SR.Text = "Runs Completed: " .. tostring(getgenv().AF_TotalRuns); UI.SGld.Text = "Gold Earned: ~" .. tostring(getgenv().AF_TotalGold) en.d)
                                hum.Health = 0; re.peat ta.sk.wait(0.05) un.til respawned; conn:Disconnect()
                            en.d
                        en.d)
                    e.lseif getgenv().TreasureAutoFarm.Mode == "Fast-16" th.en
                        pcall(fun.ction()
                            lo.cal ns = WS:WaitForChild("BoatStages", 10):WaitForChild("NormalStages", 10)
                            lo.cal st1 = ns:WaitForChild("CaveStage1", 5)
                            lo.cal dark = st1 and st1:WaitForChild("DarknessPart", 5)
                            if dark th.en
                                lo.cal tier = getgenv().TreasureAutoFarm.SpeedTier or 2
                                lo.cal serverWait = (tier == 1) and 3.8 or (tier == 2) and 2.9 or 2.2
                                hrp.CFrame = dark.CFrame
                                for i = 1, 30 do firetouchinterest(hrp, dark, 0); firetouchinterest(hrp, dark, 1) en.d
                                lo.cal tTime = 0
                                wh.ile tTime < serverWait and getgenv().TreasureAutoFarm.Mode == "Fast-16" do
                                    hrp.CFrame = dark.CFrame * CFrame.new(2, 0, 0); ta.sk.wait(0.1); hrp.CFrame = dark.CFrame * CFrame.new(-2, 0, 0); ta.sk.wait(0.1); tTime = tTime + 0.2
                                en.d
                                lo.cal respawned = false
                                lo.cal conn = LP.CharacterAdded:Connect(fun.ction() respawned = tr.ue en.d)
...
en.d)
el.se
    if getgenv().AF_TI th.en getgenv().AF_TI.Text = ""; getgenv().AF_TI.PlaceholderText = "Wrong Password!" en.d
en.d
