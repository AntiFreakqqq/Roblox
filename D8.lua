...
local PF = Instance.new("Frame", SG)
PF.Size = UDim2.new(0, 260, 0, 130)
PF.Position = UDim2.new(0.35, 0, 0.4, 0)
PF.BackgroundColor3 = Color3.fromRGB(14, 12, 20)
Instance.new("UICorner", PF).CornerRadius = UDim.new(0, 10)
local PS = Instance.new("UIStroke", PF)
PS.Color = Color3.fromRGB(140, 80, 255)

local PT = Instance.new("TextLabel", PF)
PT.Size = UDim2.new(1, 0, 0, 30)
PT.BackgroundTransparency = 1
PT.Text = "ACCESS REQUIRED"
PT.TextColor3 = Color3.fromRGB(180, 120, 255)
PT.Font = Enum.Font.GothamBold
PT.TextSize = 12

local TI = Instance.new("TextBox", PF)
TI.Size = UDim2.new(1, -40, 0, 30)
TI.Position = UDim2.new(0, 20, 0, 35)
...
local PasteBtn = Instance.new("TextButton", PF)
PasteBtn.Size = UDim2.new(0, 105, 0, 30)
PasteBtn.Position = UDim2.new(0, 20, 0, 75)
PasteBtn.BackgroundColor3 = Color3.fromRGB(30, 25, 45)
PasteBtn.Text = "📋 PASTE"
PasteBtn.TextColor3 = Color3.fromRGB(200, 180, 255)
PasteBtn.Font = Enum.Font.GothamBold
PasteBtn.TextSize = 10
Instance.new("UICorner", PasteBtn).CornerRadius = UDim.new(0, 6)

local SubBtn = Instance.new("TextButton", PF)
SubBtn.Size = UDim2.new(0, 105, 0, 30)
SubBtn.Position = UDim2.new(1, -125, 0, 75)
SubBtn.BackgroundColor3 = Color3.fromRGB(140, 75, 255)
SubBtn.Text = "✅ SUBMIT"
...
local function drag(obj)
    ...
    UIS.InputChanged:Connect(function(i) if i == di and d then local delta = i.Position - ds; obj.Position = UDim2.new(sp.X.Scale, sp.X.Offset + delta.X, sp.Y.Scale, sp.Y.Offset + delta.Y) end end)
end
drag(PF)

PasteBtn.MouseButton1Click:Connect(function()
    if getclipboard then TI.Text = getclipboard() else TI.PlaceholderText = "Not supported..." end
end)
_G.AF_SpawnMainUI = function()
    SG:Destroy()
    ...
    SC.Position = UDim2.new(0, 20, 0, 185)
    ...
    SGld.Text = "Gold Earned: 0"
    SGld.TextColor3 = Color3.fromRGB(220, 200, 255)
    SGld.Font = Enum.Font.GothamBold
    SGld.TextXAlignment = Enum.TextXAlignment.Left

    getgenv().AF_Data = { TB=TB, MM=MM, MS=MS, T=T, LS=LS, FTB=FTB, BTS=BTS, FAST=FAST, FASTS=FASTS, AFK=AFK, AFKS=AFKS, AB=AB, AS=AS, SC=SC, ST=ST, ISG=ISG, IF=IF, SR=SR, SGld=SGld, MainSG=MainSG }
end

SubBtn.MouseButton1Click:Connect(function()
    if TI.Text == "Антифрикбог" then getgenv().AF_PassVerified = true; _G.AF_SpawnMainUI() else TI.Text = ""; TI.PlaceholderText = "Wrong Password!" end
end)
                task.spawn(function()
    while task.wait(0.5) do
        if getgenv().AF_PassVerified and getgenv().AF_Data and getgenv().AF_Data.MainSG then
            local UI = getgenv().AF_Data
            local TS = game:GetService("TweenService")
            local UIS = game:GetService("UserInputService")
            
            local function makeGear(txt, pos, active)
                local g = Instance.new("TextButton", UI.SC)
            ...
            task.spawn(function() 
                while true do 
                    ...
                    if getgenv().AF_AntiAFK and getgenv().TreasureAutoFarm.Mode ~= "None" then 
                        pcall(function() game:GetService("VirtualInputManager"):SendKeyEvent(true, Enum.KeyCode.Space, false, game); task.wait(0.05); game:GetService("VirtualInputManager"):SendKeyEvent(false, Enum.KeyCode.Space, false, game) end) 
                    end 
                end 
            end)
            break
        ...
        UI.FTB.MouseButton1Click:Connect(function()
            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then 
                getgenv().TreasureAutoFarm.Mode = "None"; UI.FTB.BackgroundColor3 = Color3.fromRGB(24, 20, 32); UI.FTB.Text = "🚀 LAUNCH FLY-TP"; UI.ISG.Enabled = false
            else 
                getgenv().TreasureAutoFarm.Mode = "Fly-TP"; getgenv().AF_TotalRuns = 0; getgenv().AF_TotalGold = 0; UI.SR.Text = "Runs Completed: 0"; UI.SGld.Text = "Gold Earned: 0"; UI.ISG.Enabled = true; UI.FTB.BackgroundColor3 = Color3.fromRGB(140, 75, 255); UI.FTB.Text = "⚡ FLY-TP: ACTIVE"; UI.FAST.BackgroundColor3 = Color3.fromRGB(24, 20, 32); UI.FAST.Text = "⚡ FAST 16 FARM"
            end
        end)

        UI.FAST.MouseButton1Click:Connect(function()
            if getgenv().TreasureAutoFarm.Mode == "Fast-16" then
                getgenv().TreasureAutoFarm.Mode = "None"; UI.FAST.BackgroundColor3 = Color3.fromRGB(24, 20, 32); UI.FAST.Text = "⚡ FAST 16 FARM"; UI.ISG.Enabled = false
            else
                getgenv().TreasureAutoFarm.Mode = "Fast-16"; getgenv().AF_TotalRuns = 0; getgenv().AF_TotalGold = 0; UI.SR.Text = "Runs Completed: 0"; UI.SGld.Text = "Gold Earned: 0"; UI.ISG.Enabled = true; UI.FAST.BackgroundColor3 = Color3.fromRGB(140, 75, 255); UI.FAST.Text = "🔮 FAST-16: ACTIVE"; UI.FTB.BackgroundColor3 = Color3.fromRGB(24, 20, 32); UI.FTB.Text = "🚀 LAUNCH FLY-TP"
            end
        end)
        ...
        RS.Stepped:Connect(function()
            ...
            if stageIndex and stageIndex <= 2 then
                currentSpeed = 350; fixTime = 1.10; calculatedY = 85
            else
                ...
                if hrp and hum and hum.Health > 0 then
                        ...
                        pcall(function()
                            ...
                            for i = 2, 10 do
                                ...
                                if (getgenv().TreasureAutoFarm.SpeedTier or 2) < 3 and (i == 3 or i == 6 or i == 9 or i == 10) then
                                    hybridMoveTo(Vector3.new(stagePos.X + 18, 85, stagePos.Z), false); hybridMoveTo(Vector3.new(stagePos.X - 18, 85, stagePos.Z), false); hybridMoveTo(Vector3.new(stagePos.X, 85, stagePos.Z), i)
                                end
                            end
                            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" and ns:FindFirstChild("TheEnd") then
                                ...
                                if trig then hrp.CFrame = CFrame.new(trig.Position); pcall(function() for i = 1, 15 do firetouchinterest(hrp, trig, 0); firetouchinterest(hrp, trig, 1) end end)
                            ...
                            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then
                                ...
                                getgenv().AF_TotalRuns = getgenv().AF_TotalRuns + 1; getgenv().AF_TotalGold = getgenv().AF_TotalGold + 105
                                pcall(function() UI.SR.Text = "Runs Completed: " .. tostring(getgenv().AF_TotalRuns); UI.SGld.Text = "Gold Earned: ~" .. tostring(getgenv().AF_TotalGold) end)
                                hum.Health = 0; repeat task.wait(0.05) until respawned; conn:Disconnect()
                            end
                        end)
                    elseif getgenv().TreasureAutoFarm.Mode == "Fast-16" then
                        pcall(function()
                            ...
                            if dark then
                                local tier = getgenv().TreasureAutoFarm.SpeedTier or 2
                                local serverWait = (tier == 1) and 3.8 or (tier == 2) and 2.9 or 2.2
                                hrp.CFrame = dark.CFrame
                                for i = 1, 30 do firetouchinterest(hrp, dark, 0); firetouchinterest(hrp, dark, 1) end
                                local tTime = 0
                                while tTime < serverWait and getgenv().TreasureAutoFarm.Mode == "Fast-16" do
                                    hrp.CFrame = dark.CFrame * CFrame.new(2, 0, 0); task.wait(0.1); hrp.CFrame = dark.CFrame * CFrame.new(-2, 0, 0); task.wait(0.1); tTime = tTime + 0.2
                                end
                                local respawned = false
                                local conn = LP.CharacterAdded:Connect(function() respawned = true end)
                                getgenv().AF_TotalRuns = getgenv().AF_TotalRuns + 1; getgenv().AF_TotalGold = getgenv().AF_TotalGold + 16
                                pcall(function() UI.SR.Text = "Runs Completed: " .. tostring(getgenv().AF_TotalRuns); UI.SGld.Text = "Gold Earned: ~" .. tostring(getgenv().AF_TotalGold) end)
                                hum.Health = 0; repeat task.wait(0.05) until respawned; conn:Disconnect()
                            end
                        end)
...
TI.FocusLost:Connect(function()
    if TI.Text == "Антифрикбог" then initMainCheatUI()
    else TI.Text = ""; TI.PlaceholderText = "Wrong! Try Again..." end
end)
