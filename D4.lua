...
if getgenv().AF_SubBtn then
    getgenv().AF_SubBtn.MouseButton1Click:Connect(function()
        if getgenv().AF_TI and getgenv().AF_TI.Text == "Антифрикбог" then 
            ...
            local G2, G2S = makeGear("Gear 2", UDim2.new(0, 70, 0, 20), true, UI.SC)
            local G3, G3S = makeGear("Gear 3", UDim2.new(0, 140, 0, 20), false, UI.SC)
            ...
            local normalSize, expandedSize = UDim2.new(0, 260, 0, 180), UDim2.new(0, 260, 0, 240)
                    ...
                    if hrp and hum and hum.Health > 0 then
                        ...
                        elseif getgenv().TreasureAutoFarm.Mode == "Fast-16" then
                            pcall(function()
                                ...
                                if dark then
                                    local tier = getgenv().TreasureAutoFarm.SpeedTier or 2
                                    local serverWait = (tier == 1) and 2.5 or (tier == 2) and 1.8 or 1.2
                                    ...
                                    while tTime < serverWait and getgenv().TreasureAutoFarm.Mode == "Fast-16" do
                                        hrp.CFrame = dark.CFrame * CFrame.new(1.5, 0, 0)
                                        task.wait(0.08)
                                        hrp.CFrame = dark.CFrame * CFrame.new(-1.5, 0, 0)
                                        task.wait(0.08)
                                        tTime = tTime + 0.16
