...
getgenv().AF_StartGold = 0
getgenv().AF_StartTime = tick()
...
local function createStatsPanel(parentGui)
    local SF = Instance.new("Frame", parentGui)
    SF.Name = "StatsPanel"
    SF.Size = UDim2.new(0, 180, 0, 80)
    SF.Position = UDim2.new(0.05, 0, 0.15, 0)
    ...
    SS.Color = Color3.fromRGB(100, 60, 180)
    SS.Thickness = 1.2
    
    local GL = Instance.new("TextLabel", SF)
    GL.Size = UDim2.new(1, -20, 0, 25)
    GL.Position = UDim2.new(0, 10, 0, 10)
    GL.BackgroundTransparency = 1
    GL.Text = "💰 Gold Earned: 0"
    GL.TextColor3 = Color3.fromRGB(255, 215, 0)
    GL.TextSize = 11
    ...
    TL.Position = UDim2.new(0, 10, 0, 40)
    TL.BackgroundTransparency = 1
    TL.Text = "⏱️ Time: 00:00"
    ...
    task.spawn(function()
        while task.wait(1) do
            pcall(function()
                local currentGold = LP.Data.Gold.Value
                local earned = currentGold - getgenv().AF_StartGold
                GL.Text = "💰 Gold Earned: " .. earned
                
                local elapsed = math.floor(tick() - getgenv().AF_StartTime)
                local m = math.floor(elapsed / 60)
                local s = elapsed % 60
                TL.Text = string.format("⏱️ Time: %02d:%02d", m, s)
            end)
        end
    end)
end
local function initMainCheatUI()
    SG:Destroy()
    getgenv().AF_StartGold = LP.Data.Gold.Value
    getgenv().AF_StartTime = tick()
    ...
    FAST.Size = UDim2.new(1, -45, 0, 38)
    FAST.Position = UDim2.new(0, 0, 0, 45)
    ...
    SC.Position = UDim2.new(0, 20, 0, 140)
    ...
    local normalSize, expandedSize = UDim2.new(0, 260, 0, 135), UDim2.new(0, 260, 0, 195)
    ...
    TB.MouseButton1Click:Connect(function()
        ...
        TS:Create(FAST, ti, {BackgroundTransparency = menuOpen and 0 or 1, TextTransparency = trans}):Play()
        TS:Create(FASTS, ti, {Transparency = trans}):Play()
    ...
    FAST.MouseButton1Click:Connect(function()
        if getgenv().TreasureAutoFarm.Mode == "Fast-16" then
            ...
            FAST.BackgroundColor3 = Color3.fromRGB(140, 75, 255)
            FAST.Text = "🔮 FAST-16: ACTIVE"
            FTB.BackgroundColor3 = Color3.fromRGB(24, 20, 32)
            FTB.Text = "🚀 LAUNCH FLY-TP"
        end
    end)
                ...
                if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then
                    pcall(function()
                        ...
                        if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then
                            ...
                            pcall(function() STAT_R.Text = "Runs Completed: " .. tostring(getgenv().AF_TotalRuns) end)
                            pcall(function() STAT_G.Text = "Gold Earned: ~" .. tostring(getgenv().AF_TotalGold) end)
                            hum.Health = 0
                ...
                elseif getgenv().TreasureAutoFarm.Mode == "Fast-16" then
                    pcall(function()
                        ...
                        if dark then
                            ...
                            for i = 1, 20 do firetouchinterest(hrp, dark, 0); firetouchinterest(hrp, dark, 1) end
                            
                            -- НАШ СЕКРЕТНЫЙ ЗИГЗАГ ДЛЯ ОБХОДА ЗАЩИТЫ СЕРВЕРА (Имитируем ходьбу):
                            local tTime = 0
                            while tTime < serverWait and getgenv().TreasureAutoFarm.Mode == "Fast-16" do
                                hrp.CFrame = dark.CFrame * CFrame.new(3, 0, 0)
                                task.wait(0.1)
                                hrp.CFrame = dark.CFrame * CFrame.new(-3, 0, 0)
                                task.wait(0.1)
                                tTime = tTime + 0.2
