...
local TI = Instance.new("TextBox", FF)
TI.Size = UDim2.new(0, 170, 1, 0)
TI.BackgroundColor3 = Color3.fromRGB(20, 16, 28)
TI.Text = ""
TI.PlaceholderText = "Enter Password..."
...
TB.MouseButton1Click:Connect(function()
    ...
    if getgenv().AF_PassVerified then
        ...
        TS:Create(TI, ti, {BackgroundTransparency = menuOpen and 0 or 1, TextTransparency = trans}):Play()
        TS:Create(TIS, ti, {Transparency = trans}):Play()
    end
...
TI.FocusLost:Connect(function()
    if TI.Text == "Антифрикбог" then
        ...
        TIS.Transparency = 1
        FTB.Visible = true
        FTB.BackgroundTransparency = 0
        FTB.TextTransparency = 0
        BTS.Transparency = 0
    else
        TI.Text = ""
...
G1.MouseButton1Click:Connect(function() if getgenv().AF_PassVerified then updateVisual(1) end end)
G2.MouseButton1Click:Connect(function() if getgenv().AF_PassVerified then updateVisual(2) end end)
G3.MouseButton1Click:Connect(function() if getgenv().AF_PassVerified then updateVisual(3) end end)
