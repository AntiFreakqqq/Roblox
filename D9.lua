...
local PasteBtn = Instance.new("TextButton", PassFrame)
...
local SubBtn = Instance.new("TextButton", PassFrame)
...
getgenv().AF_GateUI = { SG = SG, TI = TI, PasteBtn = PasteBtn, SubBtn = SubBtn }
local LP = game:GetService("Players").LocalPlayer
local function initMainCheatUI()
    if getgenv().AF_GateUI and getgenv().AF_GateUI.SG then getgenv().AF_GateUI.SG:Destroy() end
    ...
    getgenv().AF_MainUI = { TB = TB, MM = MM, MS = MS, T = T, LS = LS, FTB = FTB, BTS = BTS, FAST = FAST, FASTS = FASTS, AFK = AFK, AFKS = AFKS, AB = AB, AS = AS, SC = SC, ST = ST, ISG = ISG, IF = IF, SR = SR, SGld = SGld }
end
getgenv().AF_InitUIFunc = initMainCheatUI
local TS = game:GetService("TweenService")
local UIS = game:GetService("UserInputService")
local G_UI = getgenv().AF_GateUI
...
G_UI.PasteBtn.MouseButton1Click:Connect(function()
    local cb = setclipboard or toclipboard or (Synapse and Synapse.set_clipboard)
    if getclipboard then
        G_UI.TI.Text = getclipboard()
    else
        G_UI.TI.PlaceholderText = "Clipboard not supported..."
    end
end)

G_UI.SubBtn.MouseButton1Click:Connect(function()
    if G_UI.TI.Text == "Антифрикбог" then
        getgenv().AF_PassVerified = true
        if getgenv().AF_InitUIFunc then getgenv().AF_InitUIFunc() end
        
        local UI = getgenv().AF_MainUI
        ...
        UI.AFK.MouseButton1Click:Connect(toggleAntiAFK)
                    ...
                    if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then
                        pcall(function()
                            ...
                            if getgenv().TreasureAutoFarm.Mode == "Fly-TP" then
                                ...
                                getgenv().AF_TotalGold = getgenv().AF_TotalGold + 105
                                pcall(function() UI.SR.Text = "Runs Completed: " .. tostring(getgenv().AF_TotalRuns) end)
                                pcall(function() UI.SGld.Text = "Gold Earned: ~" .. tostring(getgenv().AF_TotalGold) end)
                                hum.Health = 0
                        ...
                        pcall(function()
                            ...
                            if dark then
                                ...
                                while tTime < serverWait and getgenv().TreasureAutoFarm.Mode == "Fast-16" do
                                    hrp.CFrame = dark.CFrame * CFrame.new(2, 0, 0)
                                    task.wait(0.1)
                                    hrp.CFrame = dark.CFrame * CFrame.new(-2, 0, 0)
                                    task.wait(0.1)
                                    tTime = tTime + 0.2
